...
def train_model(sweep_q, worker_q):
    ...
    # Train the model
    train_losses, val_losses, val_mean_diffs, val_std_diffs = train(model, worker_data.train_loader, worker_data.val_loader, criterion, optimizer, run, config, device)

    # and test its final performance
    mean_diff, std_diff = test(model, worker_data.test_loader, config, device)
...
def main():
    ...
    dataset = CustomDataset(data_dir=DATA_DIR, model_arch=sweep_run.config["model_arch"], mask_type=sweep_run.config["mask_type"], transform=True)

    # split off test set
    train_val_indices, test_indices = train_test_split(
        list(range(len(dataset))), test_size=(sweep_run.config["test_size"]/100), random_state=SEED
    )
    test_dataset = torch.utils.data.Subset(dataset, test_indices)
    test_loader = make_loader(test_dataset, batch_size=sweep_run.config["batch_size"])

    # KFold Cross-validation
    kfold = KFold(n_splits=sweep_run.config["k_folds"], shuffle=True, random_state=SEED)
    ...
    for fold_num, (train_indices, val_indices) in enumerate(kfold.split(range(len(train_val_indices)))):
        ...
        train_loader = make_loader(train_dataset, batch_size=sweep_run.config["batch_size"])
        val_loader = make_loader(val_dataset, batch_size=sweep_run.config["batch_size"])
